...
import type { AppConfig, AppsConfig } from "../types.js";
...
export const loadAppsConfigFromEnv = (
  env: Record<string, string | undefined> = process.env,
): AppsConfig => {
  const appsStr = env.DISPATCH_APPS;
  if (!appsStr) {
    throw new Error(
      "DISPATCH_APPS environment variable is required (comma-separated app names)",
    );
  }

  const appNames = appsStr
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  for (const appName of appNames) {
    ...
    if (Object.keys(channels).length === 0) {
      throw new Error(
        `App "${appName}" must have at least one channel configured (e.g., ${prefix}_DISCORD_WEBHOOK or ${prefix}_SLACK_WEBHOOK)`,
      );
    }
...
export const validateAppsConfig = (config: AppsConfig): void => {
  ...
  for (const [appName, appConfig] of Object.entries(config)) {
    ...
    if (app.apiKey.length < 20) {
      throw new Error(
        `App "${appName}": apiKey must be at least 20 characters`,
      );
    }

    if (seenKeys.has(app.apiKey)) {
      throw new Error(
        `Duplicate API key found across apps. Each app must have a unique apiKey.`,
      );
    }
    ...
    if (app.channels.discord) {
      if (
        !app.channels.discord.webhookUrl ||
        typeof app.channels.discord.webhookUrl !== "string"
      ) {
        throw new Error(
          `App "${appName}": discord channel missing "webhookUrl"`,
        );
      }
    }
    if (app.channels.slack) {
      if (
        !app.channels.slack.webhookUrl ||
        typeof app.channels.slack.webhookUrl !== "string"
      ) {
...
export const buildKeyIndex = (
  config: AppsConfig,
): Map<string, { appName: string; appConfig: AppConfig }> => {
